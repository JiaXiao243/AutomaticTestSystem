...
class RepoInit():
      def __init__(self, repo):
         ...
         cmd='''git clone https://github.com/PaddlePaddle/%s.git --depth 1; cd %s; python -m pip install --upgrade pip; python -m pip install -r requirements.txt; export PYTHONPATH=$PWD:$PYTHONPATH >> ~/.bashrc; source ~/.bashrc''' % (self.repo, self.repo)
         repo_result=subprocess.getstatusoutput(cmd)
         exit_code=repo_result[0]
...
class TestClassModel():
      ...
      def test_class_get_pretrained_model(self):
          ...
          if self.model in legendary_models:
             ...
             cmd='cd PaddleClas;wget https://paddle-imagenet-models-name.bj.bcebos.com/dygraph/%s_pretrained.pdparams' % self.model
          clas_result = subprocess.getstatusoutput(cmd)
          exit_code = clas_result[0]
          output = clas_result[1]
          allure_step(cmd, output)
      ...
      def test_class_export_model(self):
          ...
          class export model
          """
          cmd='cd PaddleClas;python tools/export_model.py -c %s -o Global.pretrained_model=./%s_pretrained -o Global.save_inference_dir=./inference/%s' % (self.yaml, self.model, self.model)
          clas_result=subprocess.getstatusoutput(cmd)
          exit_code=clas_result[0]
          output=clas_result[1]
          allure_step(cmd, output)
...
class TestOcrModel():
      ...
      def test_ocr_train(self):
          cmd='cd PaddleOCR; export CUDA_VISIBLE_DEVICES=0,1,2,3; sed -i s!data_lmdb_release/training!data_lmdb_release/validation!g %s; python -m paddle.distributed.launch --gpus=0,1,2,3 --log_dir=log_%s  tools/train.py -c %s -o Global.use_gpu=True Global.epoch_num=1 Global.save_epoch_step=1 Global.eval_batch_step=200 Global.print_batch_step=10 Global.save_model_dir=output/%s Train.loader.batch_size_per_card=10 Global.print_batch_step=1' % (self.yaml,  self.model, self.yaml, self.model)
          print(cmd)
      ...
      def test_ocr_eval(self):
          cmd='cd PaddleOCR; export CUDA_VISIBLE_DEVICES=0,1,2,3;  python tools/eval.py -c %s  -o Global.use_gpu=True Global.checkpoints=output/%s/latest' % (self.yaml, self.model)
          detection_result = subprocess.getstatusoutput(cmd)
          exit_code = detection_result[0]
          output = detection_result[1]
      ...
      def test_ocr_rec_infer(self):
          cmd='cd PaddleOCR; export CUDA_VISIBLE_DEVICES=0,1,2,3; python tools/infer_rec.py -c %s  -o Global.use_gpu=True Global.checkpoints=output/%s/latest Global.infer_img=doc/imgs_words/en/word_1.png' % (self.yaml, self.model)
          detection_result = subprocess.getstatusoutput(cmd)
          exit_code = detection_result[0]
          output = detection_result[1]
      ...
      def test_ocr_export_model(self):
          cmd='cd PaddleOCR; export CUDA_VISIBLE_DEVICES=0,1,2,3; python tools/export_model.py -c %s -o Global.use_gpu=True Global.checkpoints=output/%s/latest  Global.save_inference_dir=./models_inference/%s' % (self.yaml, self.model, self.model)
          print(cmd)
          detection_result = subprocess.getstatusoutput(cmd)
          exit_code = detection_result[0]
          output = detection_result[1]
      ...
      def test_ocr_rec_predict(self):
          cmd='cd PaddleOCR; export CUDA_VISIBLE_DEVICES=0,1,2,3; python tools/infer/predict_rec.py --image_dir="./doc/imgs_words_en/word_336.png" --rec_model_dir="./models_inference/"%s --rec_image_shape="3, 32, 100" --rec_char_type="en" --rec_algorithm=CRNN' % (self.model)
          detection_result = subprocess.getstatusoutput(cmd)
          exit_code = detection_result[0]
          output = detection_result[1]
          exit_check_fucntion(exit_code, output, 'predict')
      ...
      def test_ocr_e2e_infer(self):
          cmd='cd PaddleOCR; export CUDA_VISIBLE_DEVICES=0,1,2,3; python tools/infer_e2e.py -c %s  -o Global.use_gpu=True Global.checkpoints=output/%s/latest Global.infer_img="./doc/imgs_en/img_10.jpg"' % (self.yaml, self.model)
      ...
      def test_ocr_e2e_predict(self):
          cmd='cd PaddleOCR; export CUDA_VISIBLE_DEVICES=0,1,2,3; python tools/infer/predict_e2e.py --image_dir="./doc/imgs_en/img623.jpg" --e2e_model_dir=./models_inference/%s --e2e_algorithm=PGNet --e2e_pgnet_polygon=True --use_gpu=True' % (self.model)
          e2eection_result = subprocess.getstatusoutput(cmd)
          exit_code = e2eection_result[0]
          output = e2eection_result[1]
          exit_check_fucntion(exit_code, output, 'predict')
      ...
      def test_ocr_cls_infer(self):
          cmd='cd PaddleOCR; export CUDA_VISIBLE_DEVICES=0,1,2,3; python tools/infer_cls.py -c %s  -o Global.use_gpu=True Global.checkpoints=output/%s/latest Global.infer_img="./doc/imgs_en/img_10.jpg"' % (self.yaml, self.model)
      ...
      def test_ocr_cls_predict(self):
          cmd='cd PaddleOCR; export CUDA_VISIBLE_DEVICES=0,1,2,3; python tools/infer/predict_cls.py --image_dir="./doc/imgs_en/img623.jpg" --cls_model_dir=./models_inference/%s --use_gpu=True' % (self.model)
          clsection_result = subprocess.getstatusoutput(cmd)
          exit_code = clsection_result[0]
          output = clsection_result[1]
          exit_check_fucntion(exit_code, output, 'predict')
...
class TestDetectionStaticModel():
      ...
      def test_detection_train(self):
          cmd='cd PaddleDetection/static; export CUDA_VISIBLE_DEVICES=0,1,2,3; python -m paddle.distributed.launch --gpus=0,1,2,3 --log_dir=log_%s tools/train.py -c %s -o TrainReader.batch_size=1  -o max_iters=10' % (self.model, self.yaml)
          detection_result = subprocess.getstatusoutput(cmd)
          exit_code = detection_result[0]
          output = detection_result[1]
...
class TestSegModel():
      ...
      def test_seg_eval(self):
          cmd='cd PaddleSeg; wget https://bj.bcebos.com/paddleseg/dygraph/cityscapes/%s/model.pdparams; mv model.pdparams %s.pdparams; export CUDA_VISIBLE_DEVICES=0,1,2,3; python -m paddle.distributed.launch val.py --config %s --model_path=%s.pdparams' % (self.model, self.model, self.yaml, self.model)
          detection_result = subprocess.getstatusoutput(cmd)
          exit_code = detection_result[0]
          output = detection_result[1]
          allure_step(cmd, output)
...
class TestGanModel():
      ...
      def test_gan_train(self):
          cmd='cd PaddleGAN; sed -i 1s/epochs/total_iters/ %s; export CUDA_VISIBLE_DEVICES=0,1,2,3; python -u -m paddle.distributed.launch --gpus="0,1,2,3" tools/main.py --config-file %s -o total_iters=20 snapshot_config.interval=10 log_config.interval=1 output_dir=output' % (self.yaml, self.yaml)
          print(cmd)
...
class TestParakeetModel():
      ...
      def test_parakeet_train(self):
          cmd='cd Parakeet/examples/%s; ln -s /data/ljspeech_%s ljspeech_%s; export CUDA_VISIBLE_DEVICES=0,1,2,3; python train.py --data=ljspeech_%s --output=output --device="gpu" --nprocs=4 --opts data.batch_size 2 training.max_iteration 10 training.valid_interval 10 training.save_interval 10' % (self.model, self.model, self.model, self.model)
          print(cmd)
          detection_result = subprocess.getstatusoutput(cmd)
          exit_code = detection_result[0]
          output = detection_result[1]
...
class TestRecModel():
      ...
      def test_rec_train(self):
          cmd='cd PaddleRec/%s; sed -i s/"use_gpu: False"/"use_gpu: True"/g config.yaml; python -u ../../../tools/static_trainer.py -m config.yaml' % (self.directory)
          print(cmd)
          detection_result = subprocess.getstatusoutput(cmd)
          exit_code = detection_result[0]
          output = detection_result[1]
...
class TestVideoModel():
      ...
      def test_video_train(self):
          cmd='cd PaddleVideo; python -B -m paddle.distributed.launch --gpus="0,1,2,3" main.py  --validate -c %s -o epochs=1' % (self.yaml)
          print(cmd)
          detection_result = subprocess.getstatusoutput(cmd)
          exit_code = detection_result[0]
          output = detection_result[1]
