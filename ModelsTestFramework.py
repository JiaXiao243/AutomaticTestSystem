...
class RepoInit3D():
      def __init__(self, repo):
         ...
         print("This is Repo Init!")
         pid = os.getpid()
         cmd='''cd %s; git log -5; python -m pip install -r requirements.txt --ignore-installed; python -m pip install .; python -m pip install -U urllib3==1.26.15''' % (self.repo)
         if(platform.system() == "Windows"):
               cmd=cmd.replace(';','&')
         repo_result=subprocess.getstatusoutput(cmd)
         exit_code=repo_result[0]
...
class RepoInitSpeech():
      def __init__(self, repo):
         ...
         print("This is Repo Init!")
         pid = os.getpid()
         cmd='''git clone -b r1.4 https://github.com/paddlepaddle/%s.git --depth 1; cd %s; yum update; yum install libsndfile -y;python -m pip uninstall -y paddlespeech; python -m pip install .''' % (self.repo, self.repo)
         if(platform.system() == "Windows"):
               cmd=cmd.replace(';','&')
         repo_result=subprocess.getstatusoutput(cmd)
         exit_code=repo_result[0]
...
class RepoDatasetSpeech():
      def __init__(self):
         ...
         if(sysstr =="Linux"):
            ...
            cmd='wget -c https://paddlespeech.bj.bcebos.com/PaddleAudio/cat.wav https://paddlespeech.bj.bcebos.com/PaddleAudio/dog.wav https://paddlespeech.bj.bcebos.com/PaddleAudio/zh.wav https://paddlespeech.bj.bcebos.com/PaddleAudio/en.wav https://paddlespeech.bj.bcebos.com/datasets/single_wav/zh/test_long_audio_01.wav https://paddlespeech.bj.bcebos.com/vector/audio/85236145389.wav; echo demo1 85236145389.wav > vec.job ; echo demo2 85236145389.wav >> vec.job'
         print(cmd)
...
class RepoDataset():
      def __init__(self):
         ...
         if (sysstr == "Windows"):
            cmd="rd /s /q pretrain_models & mklink /d  pretrain_models F:\PaddleOCR\pretrain_models"
         else:
...
class TestOcrModelFunction():
      ...
      def test_ocr_get_pretrained_model(self):
          ...
          if  (self.model=='det_r50_db++_icdar15') or (self.model=='det_r50_db++_td_tr'):
              cmd=self.testcase_yml['cmd'][self.category]['db++'] % (self.testcase_yml[self.model]['eval_pretrained_model'], self.tar_name, self.model, self.model)
          elif (self.category=='table') or (self.category=='kie/vi_layoutxlm') or (self.category=='e2e') or (self.category=='det'):
