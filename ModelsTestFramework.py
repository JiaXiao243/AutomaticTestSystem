...
class RepoInit():
      def __init__(self, repo):
         ...
         pid = os.getpid()
         cmd='''git clone -b dygraph https://github.com/paddlepaddle/%s.git --depth 1; cd %s; python -m pip install -r requirements.txt; python -m pip install . ; python -m pip install -r ppstructure/kie/requirements.txt; cd ..; git clone -b develop https://github.com/paddlepaddle/PaddleNLP.git --depth 1; cd PaddleNLP; python -m pip install -r requirements.txt; python -m pip install .; cd ..; git clone -b develop https://github.com/paddlepaddle/PaddleClas.git --depth 1; cd PaddleClas; python -m pip install -r requirements.txt; python -m pip install .; cd ..; python -m pip install yacs; git clone -b develop https://github.com/paddlepaddle/PaddleDetection.git --depth 1; cd PaddleDetection; python -m pip install -r requirements.txt; cd ..; cd PaddleOCR; python -m pip install -r ppstructure/recovery/requirements.txt; mkdir inference && cd inference; wget https://paddleocr.bj.bcebos.com/PP-OCRv3/english/en_PP-OCRv3_det_infer.tar && tar xf en_PP-OCRv3_det_infer.tar; wget https://paddleocr.bj.bcebos.com/PP-OCRv3/english/en_PP-OCRv3_rec_infer.tar && tar xf en_PP-OCRv3_rec_infer.tar; wget https://paddleocr.bj.bcebos.com/ppstructure/models/slanet/en_ppstructure_mobile_v2.0_SLANet_infer.tar && tar xf en_ppstructure_mobile_v2.0_SLANet_infer.tar; wget https://paddleocr.bj.bcebos.com/ppstructure/models/layout/picodet_lcnet_x1_0_fgd_layout_infer.tar && tar xf picodet_lcnet_x1_0_fgd_layout_infer.tar; cd ..''' % (self.repo, self.repo)
         if(platform.system() == "Windows"):
               cmd=cmd.replace(';','&')
