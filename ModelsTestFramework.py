...
class RepoInit():
      def __init__(self, repo):
         ...
         pid = os.getpid()
         cmd='''git clone -b dygraph https://github.com/paddlepaddle/%s.git --depth 1; cd %s; python -m pip install -r requirements.txt; python -m pip install . ; python -m pip install -r ppstructure/kie/requirements.txt; git clone -b develop https://github.com/paddlepaddle/PaddleNLP.git --depth 1; cd PaddleNLP; python -m pip install -r requirements.txt; python -m pip install .; python -m pip install yacs''' % (self.repo, self.repo)
         if(platform.system() == "Windows"):
               cmd=cmd.replace(';','&')
